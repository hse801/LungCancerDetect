...
def get_lymph_label(fPath):
    ...
    for i in range(80):
        num = '{0:0>3}'.format(i)
        if not os.path.isfile(fPath + "CT_PET_slice" + str(num) + ".txt"):
            with open("CT_PET_slice" + str(num) + ".txt", "w") as f:
                pass
            print('Create label for empty images in ', fPath)



# foldList = glob.glob('E:/HSE/LungCancerDetect/data/testset/*/')
foldList = glob.glob('E:/HSE/LungCancerDetect/data/images/train/*/')
# foldList = glob.glob('E:/HSE/LungCancerDetect/data/imags/valid/*/')
