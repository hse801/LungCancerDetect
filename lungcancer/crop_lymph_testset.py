...
def CropImage(fPath, patient_num):
    ...
    # cut the lymph node data that don't have lung_seg
    # give the certain range of x,y,z to cut
    if len(lungList) == 0:
        print('No lung data')
        print(fPath)
    ...
    #         os.chdir('E:/HSE/lymphdata/' + patient_num + '/RoiVolume_cut/')
            sitk.WriteImage(img_lymph[x_min:x_max, y_min:y_max, z_min:z_max], new_filename)
            print(new_filename, ' saved in ', os.getcwd())
    # else:
    #     img_ct = sitk.ReadImage(ct_list[0])
    #     img_ct_data = sitk.GetArrayFromImage(img_ct)
    #     img_pet = sitk.ReadImage(pet_list[0])
