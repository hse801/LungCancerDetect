...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        if paths:
            label = Path(paths[i]).name[:40]  # trim to 40 char # original
            # print(f'Path(paths[i]).name = {Path(paths[i]).name}')
            # label = Path(paths[i]).name[:8] + '_' + Path(paths[i]).name[22:] # for training image
            t_size = cv2.getTextSize(label, 0, fontScale=tl / 1.5, thickness=tf)[0]
            # cv2.putText(mosaic, label, (block_x + 15, block_y + t_size[1]), 0, tl / 1.5, [320, 320, 320], thickness=tf,
            #             lineType=cv2.LINE_AA) # for training image
            cv2.putText(mosaic, label, (block_x + 15, block_y + t_size[1] + 15), 0, tl / 1.5, [320, 320, 320], thickness=tf,
                        lineType=cv2.LINE_AA) # original
...
save_dir = 'E:/HSE/LungCancerDetect/runs/train/exp79/plots/'
